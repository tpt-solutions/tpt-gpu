...
mod tests {
    ...
    fn test_softmax_valid_2d() {
        let input =
            GpuBuffer::<f32>::new(Shape::dim2(4, 32), DType::F32, BufferFlags::STORAGE).unwrap();
    ...
    fn test_softmax_valid_1d() {
        let input =
            GpuBuffer::<f32>::new(Shape::new(&[128]), DType::F32, BufferFlags::STORAGE).unwrap();
